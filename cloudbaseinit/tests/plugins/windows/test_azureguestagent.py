...
class AzureGuestAgentPluginTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.azureguestagent = importlib.import_module(MODPATH)
        self._azure_agent = self.azureguestagent.AzureGuestAgentPlugin()
    ...
    def _test_check_delete_service(self, service_exists=True,
        ...
        self._azure_agent._check_delete_service(
            mock_osutils, mock_service_name)
        ...
        if service_exists:
            mock_osutils.get_service_status.assert_called_once_with(
                mock_service_name)
            if service_stop:
                mock_osutils.stop_service.assert_called_once_with(
                    mock_service_name, wait=True)
            else:
                self.assertEqual(mock_osutils.stop_service.call_count, 0)
            mock_osutils.delete_service.assert_called_once_with(
                mock_service_name)
        else:
    ...
    def test_remove_agent_services(self, mock_check_delete):
        ...
        with self.snatcher:
            self._azure_agent._remove_agent_services(mock_osutils)
    ...
    def test_remove_azure_dirs(self, mock_os, mock_rmtree):
        ...
        with testutils.LogSnatcher("cloudbaseinit.plugins.windows."
            ...
            self._azure_agent._remove_azure_dirs()
        self.assertEqual(mock_path.join.call_count, 2)
        self.assertEqual(mock_path.exists.call_count, 2)
        self.assertEqual(mock_rmtree.call_count, 2)

    @mock.patch('cloudbaseinit.plugins.windows.'
                'azureguestagent.winreg.REG_SZ')
    @mock.patch('cloudbaseinit.plugins.windows.'
                'azureguestagent.winreg.HKEY_LOCAL_MACHINE')
    @mock.patch('cloudbaseinit.plugins.windows.'
                'azureguestagent.winreg.SetValueEx')
    @mock.patch('cloudbaseinit.plugins.windows.'
                'azureguestagent.winreg.CreateKey')
    def test_set_registry_vm_type(self, mock_createkey,
                                  mock_setvalue, mock_HKLM,
                                  mock_reg):
        self._azure_agent._set_registry_vm_type()
        mock_createkey.assert_called_once_with(
            mock_HKLM,
            "SOFTWARE\\Microsoft\\Windows Azure")
        mock_setvalue(mock_createkey.return_value, "VMType", 0,
                      mock_reg, "IAAS")
    ...
    @mock.patch('cloudbaseinit.plugins.windows.'
                'azureguestagent.winreg.CreateKey')
    def test_set_registry_ga_params(self, mock_createkey,
                                    mock_setvalue, mock_HKLM,
                                    mock_reg):
        mock_install_timestamp = mock.Mock()
        self._azure_agent._set_registry_ga_params(
        ...
        mock_install_timestamp.strftime.assert_called_once_with(
            '%m/%d/%Y %I:%M:%S %p')
    ...
    @mock.patch('cloudbaseinit.plugins.windows.azureguestagent.'
                'datetime.datetime', side_effect=mock.sentinel)
    @mock.patch('cloudbaseinit.plugins.windows.'
                'azureguestagent.os.path.join')
    def test_configure_rd_agent(self, mock_join, mock_datetime,
                                mock_vm_type, mock_set_registry):
        mock_join.side_effect = join_sideeffect = [
            mock.sentinel,
            mock.sentinel
        ]
        mock_osutils = mock.Mock()
        ...
        self._azure_agent._configure_rd_agent(
            mock_osutils, ga_target_path)

        self.assertEqual(mock_join.call_count, 2)
        mock_osutils.create_service.assert_called_once_with(
            self.azureguestagent.SERVICE_NAME_RDAGENT,
            self.azureguestagent.SERVICE_NAME_RDAGENT,
    ...
    def _test_event_trace(self, mock_logman, action, func, mock_etcs=None):
        ...
        if mock_etcs is None:
            ...
            mock_logman.assert_called_once_with(
                mock_osutils, action, mock_name)
        else:
            func(mock_osutils, mock_name, mock_etcs)
            mock_logman.assert_called_once_with(
                mock_osutils, action, mock_name, mock_etcs)

    def test_stop_event_trace(self):
        self._test_event_trace(
            action="stop",
            func=self._azure_agent._stop_event_trace,
            mock_etcs=mock.sentinel)

    def test_delete_event_trace(self):
        self._test_event_trace(
            action="delete",
            func=self._azure_agent._delete_event_trace)
    ...
    def _test_run_logman(self, ets=False, error=True):
        ...
        mock_osutils.execute_system32_process.return_value = (out,
                                                              err,
                                                              ret_val)
        expected_logging = []
        if error:
    ...
    def _test_ga_event_traces(self, mock_stop_trace,
                              calls, expected_logging, func):
        mock_osutils = mock.Mock()
        with self.snatcher:
            func(mock_osutils)
        self.assertEqual(expected_logging, self.snatcher.output)
    ...
    def test_stop_ga_event_traces(self):
        func = self._azure_agent._stop_ga_event_traces
        expected_logging = [
    ...
    def test_delete_ga_event_traces(self):
        func = self._azure_agent._delete_ga_event_traces
        expected_logging = [
    ...
    def _test_get_guest_agent_source_path(self, mock_join,
                                          mock_exists, path_exists=True):
        mock_osutils = mock.Mock()
        count = 3
        ...
        if not path_exists:
            with self.assertRaises(exception.CloudbaseInitException) as exc:
                self._azure_agent._get_guest_agent_source_path(mock_osutils)
            self.assertEqual(
                str(exc.exception),
                "Azure guest agent source folder not found")
        else:
            result = self._azure_agent._get_guest_agent_source_path(
                mock_osutils)
    ...
    def _test_execute(self, mock_osutils_factory, mock_get_sourcepath,
        ...
        mock_provisioning = mock_service.get_vm_agent_package_provisioning_data
        mock_provisioning.return_value = provisioning_data
        if provisioning_data is None:
            ...
            mock_osutils = mock.Mock()
            mock_osutils_factory.return_value = mock_osutils
            ...
            if not ga_package_name:
                ...
                if not ga_zip_path_exists:
                    ...
                    with self.assertRaises(
                            exception.CloudbaseInitException) as exc:
        ...
        if (provisioning_data is not None and
            ...
            if ga_package_name is not None:
                if not ga_zip_path_exists:
                    ...
                    mock_zip_file.extract_all.assert_called_once_with(
                        ga_target_path)
                    mock_rd_agent.assert_called_once_with(
                        mock_osutils, ga_target_path)
                    if dotnet_installed:
                        mock_set_service = mock_osutils.set_service_start_mode
