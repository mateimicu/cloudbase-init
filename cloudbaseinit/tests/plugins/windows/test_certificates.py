...
class ServerCerificatesPluginTests(unittest.TestCase):
    def setUp(self):
        ...
        self._cert_module = importlib.import_module(module_path)

        self._cert = self._cert_module.ServerCerificatesPlugin()
    ...
    def _test_use_machine_keyset(self, local_machine=None, current_user=None):
        ...
        certificate_plugin = self._cert_module.ServerCerificatesPlugin
        if store_location != "fake store_location":
            result = certificate_plugin._use_machine_keyset(store_location)
        if store_location == constant.CERT_LOCATION_CURRENT_USER:
            ...
            with self.assertRaises(exception.ItemNotFoundException):
                self._cert_module.ServerCerificatesPlugin._use_machine_keyset(
                    store_location)
    ...
    def _test_execute(self, mock_crypto_manager, certs_info=None):
        ...
        self.assertEqual(result,
                         (self._cert_module.base.PLUGIN_EXECUTION_DONE, False))
