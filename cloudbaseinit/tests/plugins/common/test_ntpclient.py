...
class NTPClientPluginTests(unittest.TestCase):
    ...
    def _test_execute(self, mock_unpack_ntp_hosts,
        ...
        if is_real_time:
            mock_osutils.is_real_time_clock_utc.return_value = "fake time"
        else:
            mock_osutils.is_real_time_clock_utc.return_value = "fake value"
        ...
        if not is_real_time:
            mock_osutils.set_real_time_clock_utc.assert_called_once_with(
                "fake time")
            expected_logging.append('RTC set to UTC: %s' % "fake time")
        if enable_service:
